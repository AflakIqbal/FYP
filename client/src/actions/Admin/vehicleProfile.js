...
export const getProfiles = () => async dispatch => {
  ...
  try {
    const res = await axios.get('/api/Admin/viewVehicle');
...
export const getProfileById = userID => async dispatch => {
  try {
    const res = await axios.get(`/api/Admin/viewVehicleById/${userID}`);
...
export const deleteVehicleForSubAdmin = vehicleId => async dispatch => {
  if (window.confirm('Are you sure? This can not be undone')) {
    try {
      await axios.delete(`/api/Admin/deleteVehicle/${vehicleId}`);
