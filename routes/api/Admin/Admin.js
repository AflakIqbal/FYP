  ...
  async (req, res) => {
    ...
    try {
      ...
      if (admin) {
        return res
          .status(400)
          .json({ errors: [{ msg: 'Admin alreadt exixt' }] });
      }
...
// @route   post /api/subAdmin/viewSubAdmin
// @desc    get all Owner
// @access  Public
router.get('/viewSubAdmin', auth, async (req, res) => {
  try {
...
// @desc    get Owner by id
// @access  Public
router.get('/viewSubAdminById/:id', async (req, res) => {
  try {
...
router.delete('/deleteSubAdmin/:id', async (req, res) => {
  try {
    //delete owner
    const subAdmin = await SubAdmin.findByIdAndRemove(req.params.id);
    ...
    console.log(err.message);
    res.status(500).send('Server Error, SubAdmin');
  }
});
