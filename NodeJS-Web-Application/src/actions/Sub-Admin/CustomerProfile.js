...
export const getProfiles = () => async dispatch => {
  ...
  try {
    const res = await axios.get('/api/subAdmin/viewCustomer');
...
export const getProfileById = userID => async dispatch => {
  try {
    const res = await axios.get(`/api/subAdmin/viewCustomerById/${userID}`);
...
export const deleteCustomerForSubAdmin = customerId => async dispatch => {
  if (window.confirm('Are you sure? This can not be undone')) {
    try {
      await axios.delete(`/api/subAdmin/deleteCustomer/${customerId}`);
